...
- `npm run simulate:merchant-moe-anvil` deploys the real `AgentVault` and validates its allowlisted, bounded-approval swap path against real WMNT, LBQuoter, and LBRouter bytecode/state on a disposable Mantle mainnet fork.
- Fork simulation can run as a direct LBRouter call or fork-local `AgentVault.execute` call without submitting to Mantle mainnet; the Anvil fixture discards all local transactions on shutdown.
